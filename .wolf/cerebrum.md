...
- **API readiness (in `Kahoot.Api`, NOT the API layer proper):** `Common/CurrentUser` (`ICurrentUser` via `IHttpContextAccessor`, `sub` claim); `Program.cs` wires `AddHttpContextAccessor`, JWT bearer (`MapInboundClaims=false`), `AddAuthorization`, `UseAuthentication`, `UseDefaultServiceProvider(ValidateOnBuild=true)`. VERIFIED: API boots vs Postgres 17 → whole App+Infra DI graph resolves. Remaining "API layer" = controllers + `GameHub` — event↔command map in `docs/realtime-protocol.md`.
