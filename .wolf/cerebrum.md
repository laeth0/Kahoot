...
- **NOT using DDD.** Domain entities are ANEMIC — plain data classes, `public T Prop { get; set; }` only. NO constructors, factory methods, mutation methods, computed properties, validation helpers, guard clauses, or domain constants inside entities. All logic (creation, validation, state transitions, scoring, PIN generation) lives in the `Kahoot.Application` layer services/handlers.
  - NO base classes at all. Every entity is a flat POCO with its own `public Guid Id { get; set; } = Guid.CreateVersion7();`.
  - **NO auditing.** `AuditableEntity` + `AuditableEntityInterceptor` were removed; there are no `created_at`/`updated_at` columns anywhere. Only functional timestamps remain (`ExpiresAt`, `RevokedAt`, `SubmittedAt`, `LastSeenAt`, `RemovedAt`, `StartedAt`, `FinishedAt`, `CurrentQuestion*At`). Chronological ordering uses the time-ordered UUID v7 `Id`.
  - DateTime properties have **no `Utc` suffix** so C# → snake_case → DBML columns line up. UTC comes from callers + `timestamptz` mapping (`ConfigureConventions` in `KahootDbContext`).
  - `Choice.Text` is nullable (image-only choices allowed); `Choice` and `Question` have `ImageUrl` (`varchar(2048)`, nullable). `Quiz.IsPublished` gates starting a game.
...
- **Persistence (`Kahoot.Infrastructure`):** `Persistence/KahootDbContext` + one `IEntityTypeConfiguration<T>` per entity in `Persistence/Configurations/`. `EFCore.NamingConventions` `UseSnakeCaseNamingConvention()`. `xmin` optimistic concurrency on `game_sessions` via a **shadow** `uint "Version"` property mapped to column `xmin` type `xid` (NOT the anemic entity). Password hashing: `IPasswordHasher` (Application `Common/Security`) → `BcryptPasswordHasher` (`BCrypt.Net-Next`, `EnhancedHashPassword`, work factor 12, `ISingletonService`). `KahootDbContextFactory` (design-time) reads `src/Kahoot.Api/appsettings.json` (walks up dirs to find it).
- **File uploads (backend):** `IFileStorage` + `IImageUploadService` in `Kahoot.Application/Common/Storage` (+ `FileStorageOptions` bound from `FileStorage` config section, `ImageSignature` magic-byte check). `LocalFileStorage : IFileStorage, ISingletonService` in `Kahoot.Infrastructure/Storage` (dev only — Railway disk is ephemeral). `Kahoot.Api/Controllers/UploadsController` → `POST /api/uploads/images` (multipart `file`) → 201 `{ url }` (relative). `Program.cs` serves the store as static files under `PublicBasePath` and sets `FormOptions.MultipartBodyLengthLimit`. Verified: 201 + byte-identical GET roundtrip, 400 on magic-byte mismatch / disallowed type / no file.
...
- Migrations (immutable, `Kahoot.Infrastructure/Persistence/Migrations/`): `20260909075952_InitialCreate`, `20260909082015_RemoveAuditColumnsAddMediaAndPublish`.
- **Scrutor gotcha:** marker-interface (`ITransientService`/`IScopedService`/`ISingletonService`) classes are registered `AsImplementedInterfaces()` ONLY — never as the concrete type. Inject an abstraction (`IImageUploadService`), not the class. (bug-003)
- **Game session isolation** is a hard requirement: every quiz run = a new `game_sessions` row; participants/answers/scores/leaderboard are all scoped by `game_session_id`; the answer uniqueness key is `(game_session_id, question_id, participant_id)`; prior sessions are never read/mutated by a new run. The schema already satisfies this.
